...
with tf.Session() as sess:
  ...
  batch_err = numpy.zeros(int(num_epochs * train_size) // int(BATCH_SIZE*100) + 1)
  validation_err = numpy.zeros(int(num_epochs * train_size) // int(BATCH_SIZE*100) + 1)
  number_err = 0
  for step in xrange(int(num_epochs * train_size) // BATCH_SIZE):
    ...
    if step % EVAL_FREQUENCY == 0:
      ...
      start_time = time.time()
      batch_err[number_err] = error_rate(predictions, batch_labels)
      validation_err[number_err] = error_rate(eval_in_batches(validation_data, sess), validation_labels)
      print('Step %d (epoch %.2f), %.1f ms' %
            (step, float(step) * BATCH_SIZE / train_size,
             1000 * elapsed_time / EVAL_FREQUENCY))
      print('Minibatch loss: %.3f, learning rate: %.6f' % (l, lr))
      print('Minibatch error: %.1f%%'
            % batch_err[number_err])
      print('Validation error: %.1f%%' % validation_err[number_err])
      number_err += 1
      sys.stdout.flush()
  ...
  number_step = numpy.arange(1, (int(num_epochs * train_size) // int(BATCH_SIZE*100) + 2))
  plt.plot(number_step, batch_err, 'r-', number_step, validation_err, 'b-')
  plt.ylabel('%')
